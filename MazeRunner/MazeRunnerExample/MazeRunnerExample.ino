...
void setup() {
  ...
  for (int i =0; i<NUM_SENSORS; i++) //for each sensor
  {
    ...
    SensorLightThresholds[i]= SensorLightThresholds[i] *0.95;  //Decrease the value of each Sensor threshold by 15% to allow for small errors
    Serial.println(SensorLightThresholds[i]);  
  }
   ...
   // Serial.println(turnSize);
    ...
    if (sensor_values[0]>SensorLightThresholds[0]) //if left most sensor if triggered
    {
      ...
      else //else Zumo has hit a side wall
      {
        ...
        int Duration = TriggeredSensors*2; //simplify to one line. 
        delay(200*Duration); //Spend time turning based on how many sensors triggered.  
      }
    ...
    else if (sensor_values[5]>SensorLightThresholds[5])
    {
      ...
      int Duration = TriggeredSensors*2; //simplify to one line
      delay(200*Duration); //Spend time turning based on how many sensors have triggered
      }
      TimeCapt = millis()-TimeCapt;  //work out duration of reverse maneuvour
      InstructionEnd+=TimeCapt; //add duration of reverse maneuvour to Instruction End, so it doesnt effect the distance travelled forwards
    }
...
void RoomExplore(int direction)  //0 = left & 1 = right
{
